...
public class RabbitResendThread<T> extends Thread {
    ...
    private WeakReference<RabbitResendThreadDelegate<T>> delegateWeak;
    ...
    public RabbitResendThread(RabbitResendThreadDelegate<T> delegate) {
        ...
        delegateWeak = new WeakReference<>(delegate);
    }
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                try {
                    if (retryChannel == null || !retryChannel.isOpen()) {
                        retryChannel = delegateWeak.get().createChannel();
                    }
                } catch (Exception e) {
    ...
    private void resend(T msg) throws Exception {
        delegateWeak.get().doSend(retryChannel, msg);
    }
}